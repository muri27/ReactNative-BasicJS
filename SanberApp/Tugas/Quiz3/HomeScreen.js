      ...
      <View style={styles.container}>
        <View style={{ minHeight: 50, width: DEVICE.width * 0.88 + 20, marginVertical: 8 }}>
          <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
            ...
            {/* //? #Soal 1 Tambahan, Simpan userName yang dikirim dari halaman Login pada komponen Text di bawah ini */}
              <Text style={styles.headerText}>{Login.state.userName}</Text>
            </Text>
        ...
        */}
          ...
          <View style={{flex:1}}>
                    ...
                    data = {data.produk}
                    renderItem = {({item}) =>
                        <View style ={{justifyContent:'center', alignItems:'center', width:200, height:250}}>
                            <Image source={{uri:item.gambaruri}} style={{width:100, height:100}}/>
                            <Text>{item.nama}</Text>
                            <Text>{item.harga}</Text>
                            <Text>Sisa stok: {item.stock}</Text>
                        </View>
                    }
                    numColumns={2}
